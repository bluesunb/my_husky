...
from test.robot import HuskyFR3Robot
...
for _ in range(300):
    ...
    # my_husky.set_joint_positions(action.joint_positions)
    # my_husky.set_joint_velocities(action.joint_velocities)
...
while simulation_app.is_running():
    ...
    if my_world.is_playing():
        ...
        if 0 <= i < 12:
            ...
            vel = np.zeros(13)
            vel[-4:] = 0.2
            my_husky.set_joint_velocities(vel)
            ...
            for _ in range(100):
                ...
                current_joint_positions = my_husky.get_joint_positions()[:7]
                ...
                robot_pos = my_husky.get_joint_positions()
                robot_pos[:7] = joint_action.joint_positions
                robot_vel = np.zeros(13)
                robot_vel[:7] = joint_action.joint_velocities
                my_husky.set_joint_positions(robot_pos)
                my_husky.set_joint_velocities(robot_vel)
                my_world.step(render=True)
