...
class HuskyFR3Task(ABC, BaseTask):
    """[summary]
    ...
    def __init__(
        ...
        if self._cube_initial_position is None:
            self._cube_initial_position = np.array([1.3, 0.3, 0.3]) / get_stage_units()
    ...
    def set_robot(self) -> HuskyFR3Robot:
        """[summary]

        Returns:
            Franka: [description]
        """
        husky_fr3_prim_path = find_unique_string_name(
            initial_name="/World/husky", is_unique_fn=lambda x: not is_prim_path_valid(x)
        )
        husky_fr3_robot_name = find_unique_string_name(
            initial_name="my_robot", is_unique_fn=lambda x: not self.scene.object_exists(x)
        )
        return HuskyFR3Robot(prim_path=husky_fr3_prim_path,
                             name=husky_fr3_robot_name,
    ...
    def get_observations(self) -> dict:
        ...
        return {
            ...
            self._robot.name: {
                ...
                "joint_positions": joints_state.positions[self._robot.joint_dof_indices][:7],
                "end_effector_position": end_effector_position[[0, 2, 1]],
            },
        }
